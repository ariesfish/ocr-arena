...
from ..utils.layout_postprocess_utils import apply_layout_postprocess
from ..utils.logging import get_logger
from ..utils.visualization_utils import save_layout_visualization
...
class PPDocLayoutDetector(BaseLayoutDetector):
    ...
    def stop(self):
        ...
        if self._model is not None:
            if self._device.startswith("cuda"): # type: ignore
    ...
    def _apply_per_class_threshold(self, raw_results: List[Dict]):
        ...
        label2id = {name: int(cls_id) for cls_id, name in self.id2label.items()}    # type: ignore

        # Build a lookup mapping class_id (int) -> threshold (float).
        class_thresholds = {}
        for key, value in self.threshold_by_class.items():  # type: ignore
    ...
    def process(
        ...
        for chunk_start in range(0, num_images, self.batch_size):
            ...
            inputs = self._image_processor(images=chunk_pil, return_tensors="pt")   # type: ignore
            ...
            raw_results = self._image_processor.post_process_object_detection(  # type: ignore
                outputs,
            ...
            paddle_format_results = apply_layout_postprocess(
                ...
                layout_unclip_ratio=self.layout_unclip_ratio,   # type: ignore
                layout_merge_bboxes_mode=self.layout_merge_bboxes_mode,
            )
            all_paddle_format_results.extend(paddle_format_results)

            if self._device.startswith("cuda") and chunk_end < num_images:  # type: ignore
        ...
        for img_idx, paddle_results in enumerate(all_paddle_format_results):
            ...
            for item in paddle_results:
                ...
                box = item["coordinate"]
                task_type = None
                for task_item, labels in self.label_task_mapping.items():   # type: ignore
